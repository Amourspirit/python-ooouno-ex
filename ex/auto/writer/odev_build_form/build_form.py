...
class BuildForm(
    ...
    def create_form(self, doc: XTextDocument) -> None:
        ...
        props = Forms.add_labelled_control(
            doc=BuildForm.doc,
            label="FIRSTNAME",
            comp_kind=Forms.CompenentKind.TextField,
            y=11,
        )
        ...
        props = Forms.add_labelled_control(
            doc=BuildForm.doc,
            label="AGE",
            comp_kind=Forms.CompenentKind.NumericField,
            y=43,
        )
        ...
        props = Forms.add_button(
            doc=DOC, name="first", label="<<", x=x + 0 * spacing, y=y, width=width
        )
        self.listen_to_button(props)

        props = Forms.add_button(
            doc=DOC, name="prev", label="<", x=x + 1 * spacing, y=y, width=width
        )
        self.listen_to_button(props)

        props = Forms.add_button(
            doc=DOC, name="next", label=">", x=x + 2 * spacing, y=y, width=width
        )
        self.listen_to_button(props)

        props = Forms.add_button(
            doc=DOC, name="last", label=">>", x=x + 3 * spacing, y=y, width=width
        )
        self.listen_to_button(props)

        props = Forms.add_button(
            doc=DOC, name="new", label=">*", x=x + 4 * spacing, y=y, width=width
        )
        self.listen_to_button(props)
