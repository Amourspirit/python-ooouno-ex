...
from .dispatch_reset_original_value import DispatchResetOriginalValue
from .dispatch_update_original_value import DispatchUpdateOriginalValue
...
class DispatchProviderInterceptor(unohelper.Base, XDispatchProviderInterceptor):
    ...
    def queryDispatch(
        ...
        if url.Main == ".uno:ooodev.calc.menu.reset.orig":
            with contextlib.suppress(Exception):
                args = self._convert_query_to_dict(url.Arguments)
                return DispatchResetOriginalValue(
                    sheet=args["sheet"], cell=args["cell"]
                )
        elif url.Main == ".uno:ooodev.calc.menu.update.orig":
            with contextlib.suppress(Exception):
                args = self._convert_query_to_dict(url.Arguments)
                return DispatchUpdateOriginalValue(
                    sheet=args["sheet"], cell=args["cell"]
                )
