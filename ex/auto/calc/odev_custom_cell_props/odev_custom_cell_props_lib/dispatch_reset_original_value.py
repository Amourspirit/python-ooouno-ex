...
class DispatchResetOriginalValue(unohelper.Base, XDispatch):
    ...
    def dispatch(self, url: URL, args: Tuple[PropertyValue, ...]) -> None:
        ...
        if cell.has_custom_property("OriginalValue"):
            cell.value = cell.get_custom_property("OriginalValue")
            logger.debug(
                f"Cell {self._cell} has been reset to its original value of {cell.value}."
            )
        else:
            logger.error(
                f"Cell {self._cell} does not have OriginalValue custom property."
            )
